...
export function MainScreen() {
  ...
  const [editModal, setEditModal] = useState<{
    open: boolean;
    welder: Welder | null;
  }>({ open: false, welder: null });
  const [editName, setEditName] = useState("");
  const [infoModal, setInfoModal] = useState<{
    open: boolean;
    welderId: number | null;
  }>({ open: false, welderId: null });
  ...
  const getTodaySummary = useCallback(
    (welderId: number): string => {
      ...
      for (const entry of todayEntries) {
        ...
        articleMap.set(
          entry.article,
          roundToHundredths(current + entry.quantity),
        );
      }
  ...
  // Shows most recently changed items on top
  const getWelderSummary = useCallback(
    (
      welderId: number,
    ): {
      article: string;
      ...
      const groupMap = new Map<
        string,
        { article: string; qty: number; planId: number; lastUpdatedAt: Date }
      >();

      for (const entry of entries) {
        const key = `${entry.article}__${entry.planId}`;
        const entryUpdatedAt = new Date(entry.updatedAt);
        const existing = groupMap.get(key);

        if (existing) {
          existing.qty = roundToHundredths(existing.qty + entry.quantity);
          if (entryUpdatedAt.getTime() > existing.lastUpdatedAt.getTime()) {
            existing.lastUpdatedAt = entryUpdatedAt;
          }
        } else {
          groupMap.set(key, {
            article: entry.article,
            qty: roundToHundredths(entry.quantity),
            planId: entry.planId,
            lastUpdatedAt: entryUpdatedAt,
          });
        }
      }
      ...
      for (const [, data] of groupMap) {
        ...
        if (plan) {
          if (plan.completedAt) {
            const startStr = formatDateShort(
              dateToStr(new Date(plan.createdAt)),
            );
            const endStr = formatDateShort(
              dateToStr(new Date(plan.completedAt)),
            );
            planLabel =
          ...
          if (planUpdatedAt.getTime() > lastUpdatedAt.getTime()) {
            lastUpdatedAt = planUpdatedAt;
          }
        }
      ...
      return result.sort((a, b) => {
        const dateDiff = b.lastUpdatedAt.getTime() - a.lastUpdatedAt.getTime();
  ...
  const handleWelderClick = useCallback(
    (welderId: number) => {
  ...
  const handleExport = useCallback(async () => {
    try {
      ...
      toast({
        ...
        description: `Файл welder-tracker-${today}.json скачан`,
      });
    } catch {
  ...
  return (
    <div className="flex flex-col h-full">
                ...
                >
                  ...
                  >
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setInfoModal({ open: true, welderId: welder.id! });
                      }}
      ...
      <Dialog
        open={editModal.open}
        onOpenChange={(open) => setEditModal({ open, welder: null })}
      >
      ...
      <Dialog
        open={infoModal.open}
        onOpenChange={(open) => setInfoModal({ open, welderId: null })}
      >
                ...
                return (
                  <div className="space-y-1.5">
                    {summary.map((item, idx) => (
                      <div
                        key={idx}
                        className="flex items-baseline justify-between text-sm gap-1"
                      >
                        ...
                        <div className="flex items-baseline gap-2 shrink-0">
                          <span className="font-mono">
                            {formatQtyShort(item.qty)} шт
                          </span>
                          {item.hours > 0 && (
                            <span className="text-muted-foreground text-xs">
                              {formatQtyShort(item.hours)} ч
                            </span>
                          )}
                        </div>
                      </div>
