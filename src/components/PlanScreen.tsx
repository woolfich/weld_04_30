...
export function PlanScreen() {
  ...
  return (
    <div className="flex flex-col h-full">
          ...
          <div className="divide-y divide-border">
            {sortedPlans.map((plan) => {
              ...
              return (
                ...
                >
                  <div
                    className={`flex items-center px-4 py-3 gap-2 ${isComplete ? "bg-green-50 dark:bg-green-950/30" : "active:bg-accent/50"}`}
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1 text-sm">
                        ...
                        <span className="text-muted-foreground flex-1 text-center tracking-widest text-xs">
                          · · · ·
                        </span>
                        <span className="font-mono flex items-baseline gap-0.5">
                          <span className="text-blue-800 dark:text-blue-400">
                            {formatQtyShort(plan.targetQty)}
                          </span>
                          <span className="text-blue-800 dark:text-blue-400">
                            /
                          </span>
                      ...
                      {!isComplete && norm && (
                        <div className="text-[10px] text-muted-foreground mt-0.5">
                          <span className="text-blue-800 dark:text-blue-400">
                            {formatQtyShort(
                              calcHours(plan.targetQty, norm.timeHours),
                            )}
                          </span>
                          {" / "}
                          {formatQtyShort(
                            calcHours(completed, norm.timeHours),
                          )}{" "}
                          ч
                        </div>
                      )}
      ...
      <Dialog
        open={editModal.open}
        onOpenChange={(open) => setEditModal({ open, plan: null })}
      >
      ...
      <Dialog
        open={infoModal.open}
        onOpenChange={(open) => setInfoModal({ open, plan: null })}
      >
        <DialogContent className="max-w-[300px]">
          <DialogHeader>
          ...
          {infoModal.plan?.createdAt && (
            <div className="text-xs text-muted-foreground border-t pt-2 mt-1">
              Создан:{" "}
              {new Date(infoModal.plan.createdAt).toLocaleDateString("ru")}
              {infoModal.plan.completedAt && (
                <>
                  {" "}
                  · Выполнен:{" "}
                  {new Date(infoModal.plan.completedAt).toLocaleDateString(
                    "ru",
                  )}
                </>
              )}
